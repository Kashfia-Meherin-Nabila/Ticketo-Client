  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      customClass: {
        ...
        cancelButton: "px-4 py-2 rounded-xl text-sm font-medium border border-gray-700",
      },
    }).then(async (result) => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F17] text-gray-100 p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#131927] p-6 rounded-2xl border border-gray-800/80 shadow-xl">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-wide">Manage Events</h1>
          <p className="text-sm text-gray-400 mt-1">
            Track approval status, edit listings, and manage capacity for all your events.
          </p>
        </div>
        <Link
          href="/dashboard/organizer/add-event"
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white font-medium rounded-xl transition shadow-lg shadow-pink-500/20 text-sm"
        >
      ...
      <div className="bg-[#131927] rounded-2xl border border-gray-800/80 shadow-xl overflow-hidden">
        {isLoading ? (
          <div className="p-16 text-center text-gray-400 space-y-3">
            ...
            <p className="text-sm">Loading your organization's events...</p>
          </div>
        ) : events.length === 0 ? (
              ...
              <tbody className="divide-y divide-gray-800/60 text-sm">
                {events.map((event) => (
                  <tr key={event._id} className="hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-4">
                        <img
                          ...
                          alt={event.title}
                          className="w-14 h-14 rounded-xl object-cover border border-gray-700/60 bg-gray-900 flex-shrink-0"
                        />
                        <div className="space-y-0.5">
                          <p className="font-medium text-white line-clamp-1">{event.title}</p>
                          <p className="text-xs text-gray-500 font-mono">ID: {event._id}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Ticket Price & Seats */}
                    <td className="py-4 px-6 whitespace-nowrap">
                      <div className="text-white font-semibold">${event.ticketPrice}</div>
                      <div className="text-xs text-gray-400 mt-0.5">{event.seats} seats available</div>
                    </td>
